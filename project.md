...
- Coach IME behavior now uses a simpler inset-first approach (instead of manual lift math): the overlay container applies IME insets, the panel grows taller while typing, and the input row stays directly above the keyboard without a dead gap. This was re-verified on a connected Pixel-sized device.
