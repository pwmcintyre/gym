...
- Coach IME behavior was tightened: opening the keyboard now lifts the overlay upward via IME insets instead of compressing the panel height, the panel expands taller while typing, and the upward lift is capped so the panel stays just below the status bar (re-verified on a connected Pixel-sized device).
