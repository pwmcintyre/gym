...
- Coach IME behavior now uses a simpler inset-first approach (instead of manual lift math): the panel keeps a stable height profile while typing, and the input row applies `imePadding()` so it stays above the keyboard. This was re-verified on a connected Pixel-sized device.
